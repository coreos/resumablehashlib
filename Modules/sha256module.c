...
PyDoc_STRVAR(SHA256_state__doc__,
"Get back a list representing the internal state.");

static PyObject *
SHA256_state(SHAobject *self, PyObject *unused)
{
    ...
    PyList_SetItem(state, 5, PyInt_FromLong(self->digestsize));

    return state;
}

PyDoc_STRVAR(SHA256_set_state__doc__,
"Restore the internal state of the hash using the supplied list.");

static PyObject *
SHA256_set_state(SHAobject *self, PyObject *statelist)
{
    ...
    if (PyList_Size(statelist) != 6) {
        PyErr_SetString(PyExc_ValueError, "State list must have a length of 6.");
        return NULL;
    }

    PyObject * digest = PyList_GetItem(statelist, 0);
    if (PyList_Size(digest) != 8) {
        PyErr_SetString(PyExc_ValueError, "First list argument must be a list with 8 values.");
        return NULL;
    }
    ...
    int candidateDigestSize = (int)PyInt_AsLong(PyList_GetItem(statelist, 5));
    if (candidateDigestSize != 32 && candidateDigestSize != 28) {
        PyErr_SetString(PyExc_ValueError, "Invalid digest size.");
        return NULL;
    }
    self->digestsize = candidateDigestSize;
...
static PyMethodDef SHA_methods[] = {
    ...
    {"state",     (PyCFunction)SHA256_state,     METH_NOARGS,  SHA256_state__doc__},
    {"set_state", (PyCFunction)SHA256_set_state, METH_O,       SHA256_set_state__doc__},
    {NULL,	  NULL}		/* sentinel */
};
